// process two polarization channels of PSOCT and outputs amp and phase for both channels
...
int main(int argc, char **argv) {
	...
	h_processed_signal = (data_t2 *)malloc(sizeof(data_t2) * (long long int)length_processed*4);  //200*1100*1250 of cross amp, 200*1100*1250 of cross phase,200*1100*1250 of co amp,200*1100*1250 of co phase,
	...
	if (fp != NULL) {
		...
		for (i = NALINES * DEPTH * NBLINES*2; i < NALINES * DEPTH * NBLINES * 2+NALINES * DEPTH * 20; i += 4)
			fprintf(fp, "%d,%d,%d,%d\n", h_processed_signal[i], h_processed_signal[i + 1], h_processed_signal[i + 2], h_processed_signal[i + 3]);
...
void PSOCT_process(data_t2 *h_signal, data_t2 *h_processed_signal, int z0, int DEPTH, int a, int b, int NALINES, int NBLINES, int SAMPLES) {
    void my_cufft(data_t2 *d_data, data_t *dispersion, data_t *hann, data_t2 *d_processed_cross_amp, data_t2 *d_processed_cross_phase, data_t2 *d_processed_co_amp, data_t2 *d_processed_co_phase, int z0, int DEPTH, int numBline, int NALINES);
    long long int i;
	...
	data_t2 *d_processed_cross_phase;
	data_t2   *d_processed_co_amp;
	data_t2 *d_processed_co_phase;
    uint16_t *d_data;
	...
	checkCudaErrors(cudaMalloc((void **)&d_processed_cross_phase, (long long int)NALINES * size[1] * sizeof(data_t2) * DEPTH));
	checkCudaErrors(cudaMalloc((void **)&d_processed_co_amp, (long long int)NALINES * size[1] * sizeof(data_t2) * DEPTH));
	checkCudaErrors(cudaMalloc((void **)&d_processed_co_phase, (long long int)NALINES * size[1] * sizeof(data_t2) * DEPTH));
	...
	for (int iter = 1; iter <= n; iter++) {
		...
		my_cufft(d_data, d_dispersion, d_hann, d_processed_cross_amp, d_processed_cross_phase, d_processed_co_amp, d_processed_co_phase, z0, DEPTH, size[iter] * 2, NALINES);
		...
		checkCudaErrors(cudaMemcpy(h_processed_signal + (long long int)NALINES * (NBLINES + pre_data) * (DEPTH), d_processed_cross_phase, NALINES * size[iter] * sizeof(data_t2) * (DEPTH),
			cudaMemcpyDeviceToHost));
		checkCudaErrors(cudaMemcpy(h_processed_signal + (long long int)NALINES * (NBLINES * 2 + pre_data) * (DEPTH), d_processed_co_amp, NALINES * size[iter] * sizeof(data_t2) * (DEPTH),
			cudaMemcpyDeviceToHost));
		checkCudaErrors(cudaMemcpy(h_processed_signal + (long long int)NALINES * (NBLINES * 3 + pre_data) * (DEPTH), d_processed_co_phase, NALINES * size[iter] * sizeof(data_t2) * (DEPTH),
			cudaMemcpyDeviceToHost));
	...
	checkCudaErrors(cudaFree(d_processed_cross_phase));
	checkCudaErrors(cudaFree(d_processed_co_amp));
	checkCudaErrors(cudaFree(d_processed_co_phase));
...
static __global__ void calc_amp(data_t *FFT_buffer, int num_Blines_per_FFT, int part, data_t2 *d_processed_cross_amp, data_t2 *d_processed_cross_phase, data_t2 *d_processed_co_amp, data_t2 *d_processed_co_phase, int z0, int DEPTH, int NALINES) {
	const int blockID = blockIdx.x + blockIdx.y*gridDim.x;
	const int threadID = blockID * (blockDim.x*blockDim.y) + threadIdx.y*blockDim.x + threadIdx.x;
	const int numThreads = blockDim.x * blockDim.y * gridDim.x * gridDim.y;
	...
	for (i = threadID; i < range; i += numThreads) {
		...
		long long int d = (long long int)t*NALINES * 2 * 4096 + (long long int)((i - (long long int)t*NALINES*DEPTH) / (DEPTH)) * 4096 + ((i % ((int)(DEPTH))) + z0) * 2; //cooresponding index for FFT_buffer
			...
			int sign_a1_real = (FFT_buffer[d] > 0) - (FFT_buffer[d] < 0);
			int sign_a1_imag = (FFT_buffer[d + 1] > 0) - (FFT_buffer[d + 1] < 0);
			a2 = (data_t)(pow(FFT_buffer[d +  B_step], 2) + pow(FFT_buffer[d +  1 + B_step], 2));
			int sign_a2_real = (FFT_buffer[d + B_step] > 0) - (FFT_buffer[d + B_step] < 0);
			int sign_a2_imag = (FFT_buffer[d + 1 + B_step] > 0) - (FFT_buffer[d + 1 + B_step] < 0);
			...
			d_processed_cross_phase[i + step] = data_t2((p1+PI) / 2/PI * 65535);
			d_processed_co_amp[i + step] = data_t2(sqrt(a2) / 4 * 65535);
			d_processed_co_phase[i + step] = data_t2((p2+PI) / 2/PI * 65535);

	}
}

void my_cufft(data_t2 *d_signal, data_t *dispersion, data_t *hann, data_t2 *d_processed_cross_amp, data_t2 *d_processed_cross_phase, data_t2 *d_processed_co_amp, data_t2 *d_processed_co_phase, int z0, int DEPTH, int numBline, int NALINES) {
    ...
    for(part=0; part< numBline/num_Blines_per_FFT; part++){
        ...
        //checkCudaErrors(cufftExecC2C(plan, reinterpret_cast<cufftComplex *>(FFT_buffer), reinterpret_cast<cufftComplex *>(FFT_buffer), CUFFT_FORWARD));
		//printf_data << <a, b >> > (FFT_buffer, length_fft);
		calc_amp << <dimBlock, dimGrid>> > ((data_t *)FFT_buffer, num_Blines_per_FFT, part, d_processed_cross_amp, d_processed_cross_phase, d_processed_co_amp, d_processed_co_phase, z0, DEPTH, NALINES);
