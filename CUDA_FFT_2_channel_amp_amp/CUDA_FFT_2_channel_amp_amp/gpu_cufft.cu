// process two polarization channels of PSOCT and outputs amps for both channels
...
int main(int argc, char **argv) {
    ...
    long long int length_processed = (long long int)(DEPTH) * (long long int)NALINES * (long long int)NBLINES * (long long int)2;


    h_signal = (data_t2 *)malloc(sizeof(data_t2) * (long long int)length_raw);   //1440*1100*1250*2
	h_processed_signal = (data_t2 *)malloc(sizeof(data_t2) * (long long int)length_processed);  //

	// init raw data
	for (i = 0; i < length_raw; i++) {
		h_signal[i] = (data_t2)((sin(2 * PI*signal_f*i / sample_f + PI / 6) + 1) / 2 * 65535);
	}
	...
	if (fp != NULL) {
		...
		for (i = NALINES*DEPTH*NBLINES; i < NALINES*DEPTH*NBLINES+NALINES * DEPTH * 20; i += 4)
			fprintf(fp, "%d,%d,%d,%d\n", h_processed_signal[i], h_processed_signal[i + 1], h_processed_signal[i + 2], h_processed_signal[i + 3]);
		//printf("%d,%d,%d,%d\n", h_processed_signal[i], h_processed_signal[i + 1], h_processed_signal[i + 2], h_processed_signal[i + 3]);
		fclose(fp);
...
void PSOCT_process(data_t2 *h_signal, data_t2 *h_processed_signal, int z0, int DEPTH, int a, int b, int NALINES, int NBLINES, int SAMPLES) {
    void my_cufft(data_t2 *d_data, data_t *dispersion, data_t *hann, data_t2 *d_processed_cross_amp, data_t2 *d_processed_co_amp, int z0, int DEPTH, int numBline, int NALINES);
    long long int i;
	...
	for (int iter = 1; iter <= n; iter++) {
		...
		printf("pre_data: %d\n", pre_data);
		printf("size: %d\n", size[iter]);
		checkCudaErrors(cudaMemcpy(d_data, h_signal + (long long int)NALINES * pre_data * SAMPLES * 2, (long long int)NALINES * size[iter] * sizeof(data_t2) * SAMPLES * 2, cudaMemcpyHostToDevice));
		my_cufft(d_data, d_dispersion, d_hann, d_processed_cross_amp, d_processed_co_amp, z0, DEPTH, size[iter] * 2, NALINES);
		checkCudaErrors(cudaMemcpy(h_processed_signal + (long long int)NALINES * pre_data * DEPTH, d_processed_cross_amp, NALINES * size[iter] * sizeof(data_t2) * DEPTH, cudaMemcpyDeviceToHost));
		checkCudaErrors(cudaMemcpy(h_processed_signal + (long long int)NALINES * (NBLINES + pre_data) * DEPTH, d_processed_co_amp, NALINES * size[iter] * sizeof(data_t2) * DEPTH, cudaMemcpyDeviceToHost));
	}
    //free GPU memory
...
static __global__ void calc_amp(data_t *FFT_buffer, int num_Blines_per_FFT, int part, data_t2 *d_processed_cross_amp, data_t2 *d_processed_co_amp, int z0, int DEPTH, int NALINES) {
	const int blockID = blockIdx.x + blockIdx.y*gridDim.x;
	const int threadID = blockID * (blockDim.x*blockDim.y) + threadIdx.y*blockDim.x + threadIdx.x;
	const int numThreads = blockDim.x * blockDim.y * gridDim.x * gridDim.y;
...
void my_cufft(data_t2 *d_signal, data_t *dispersion, data_t *hann, data_t2 *d_processed_cross_amp, data_t2 *d_processed_co_amp, int z0, int DEPTH, int numBline, int NALINES) {
    ...
    for(part=0; part< numBline/num_Blines_per_FFT; part++){
        ...
        //checkCudaErrors(cufftExecC2C(plan, reinterpret_cast<cufftComplex *>(FFT_buffer), reinterpret_cast<cufftComplex *>(FFT_buffer), CUFFT_FORWARD));
		//printf_data << <a, b >> > (FFT_buffer, length_fft);
		calc_amp << <dimBlock, dimGrid>> > ((data_t *)FFT_buffer, num_Blines_per_FFT, part, d_processed_cross_amp, d_processed_co_amp, z0, DEPTH, NALINES);
