...
void runTest(uint16_t *h_signal, float *h_trimmed_signal, uint16_t *d_signal, float *d_padded_signal, float *d_FFT_signal, float *d_trimmed_signal, cufftHandle *plans, cudaStream_t *streams, int num_aline, int start, int depth, uint16_t *curve) {
	...
	for (int i = 0; i < NUM_STREAMS; i++) {
		//printf("i is %d      ", i);
		checkCudaErrors(cudaMemcpyAsync(d_signal + i * 1440 * 2 * num_aline * NUM_BLINES / NUM_STREAMS, h_signal + i * 1440 * 2 * num_aline * NUM_BLINES / NUM_STREAMS, 1440 * 2 * num_aline * sizeof(uint16_t) *NUM_BLINES / NUM_STREAMS, cudaMemcpyHostToDevice, streams[i])); //use same or different streams for memcopy and computation
		//Pad Data
		//checkCudaErrors(cudaEventRecord(Start, 0));
		//dim3 threadsPerBlock(32, 32);

		PadData << <16, 1024, 0, streams[i] >> > (d_padded_signal + i * 2048 * 2 * num_aline * NUM_BLINES / NUM_STREAMS, d_signal + i * 1440 * 2 * num_aline * NUM_BLINES / NUM_STREAMS, num_aline);
		...
		//checkCudaErrors(cudaEventRecord(Start, 0));
		checkCudaErrors(cufftExecR2C(plans[i], reinterpret_cast<cufftReal *>(d_padded_signal + i * 2048 * 2 * num_aline * NUM_BLINES / NUM_STREAMS),
		...
		//checkCudaErrors(cudaEventRecord(Start, 0));
		ComplexPointwiseCalc << <16, 1024, 0, streams[0] >> > (d_trimmed_signal + i * 2 * depth * 2 * num_aline * NUM_BLINES / NUM_STREAMS, d_FFT_signal + i * 2050 * 2 * num_aline * NUM_BLINES / NUM_STREAMS, num_aline, start, depth, curve + i * num_aline * NUM_BLINES / NUM_STREAMS);
		//checkCudaErrors(cudaEventRecord(Stop, 0));
		//cudaEventSynchronize(Stop);
		...
		checkCudaErrors(cudaMemcpyAsync(h_trimmed_signal + i * 2 * depth * 2 * num_aline * NUM_BLINES / NUM_STREAMS, d_trimmed_signal + i * 2 * depth * 2 * num_aline * NUM_BLINES / NUM_STREAMS, depth * 2 * 2 * num_aline * sizeof(float) * NUM_BLINES / NUM_STREAMS, cudaMemcpyDeviceToHost, streams[i]));
	}
	//printf("\n");
