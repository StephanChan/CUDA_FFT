...
extern "C" __declspec(dllexport) void PSOCT_process(uint16_t *h_signal, data_t2 *h_processed_signal, int z0, int DEPTH, int a, int b, int NALINES, int NBLINES, int SAMPLES);
int main(int argc, char **argv) {
	...
	PSOCT_process(h_signal, h_processed_signal, 0,DEPTH,0,0,NALINES, NBLINES, SAMPLES);
...
void PSOCT_process(uint16_t *h_signal, data_t2 *h_processed_signal, int z0, int DEPTH, int a, int b, int NALINES, int NBLINES, int SAMPLES) {
    void my_cufft(uint16_t *h_signalA, data_t *dispersion, data_t *hann, data_t2 *h_processed_signal, int z0, int DEPTH, int numBline, int NALINES);
    long long int i;
	...
	checkCudaErrors(cudaMalloc((void **) &d_dataA, (long long int)NALINES * (long long int)size[1] * (long long int)sizeof(data_t2) * (long long int)SAMPLES*1));
	checkCudaErrors(cudaMalloc((void **) &d_processed, (long long int)NALINES * size[1] * sizeof(data_t2) * 1 * DEPTH));
	...
	for (int iter = 1; iter <= n; iter++) {
		...
		my_cufft(d_dataA, d_dispersion, d_hann, d_processed, z0, DEPTH, size[iter], NALINES);
		checkCudaErrors(cudaMemcpy(h_processed_signal + (long long int)NALINES *pre_data * DEPTH, d_processed, (long long int)NALINES * size[iter] * sizeof(data_t2) * DEPTH, cudaMemcpyDeviceToHost));
	}
    //free GPU memory
...
static __global__ void calc_amp(data_t *FFT_buffer, int num_Blines_per_FFT, int part, data_t2 *d_processed, int z0, int DEPTH, int NALINES) {
	const int blockID = blockIdx.x + blockIdx.y*gridDim.x;
	const int threadID = blockID * (blockDim.x*blockDim.y) + threadIdx.y*blockDim.x + threadIdx.x;
	const int numThreads = blockDim.x * blockDim.y * gridDim.x * gridDim.y;
	...
	for (i = threadID; i < range; i += numThreads) {
		...
		d_processed[k + step] = data_t2(sqrt(a)/4*65535);
	}
}

void my_cufft(uint16_t *d_signal, data_t *dispersion, data_t *hann, data_t2 *d_processed, int z0, int DEPTH, int numBline, int NALINES) {
